...
init () =
    ( { email = "", password = "", isSubmittingForm = False, errors = [] }
    , Effect.none
    )
...
update msg model =
    case msg of
        ...
        UserSubmittedForm ->
            ...
            , Api.SignIn.post
                { onResponse = SiginInApiResponded
                , email = model.email
...
viewFormControls model =
    Html.div [ Attr.class "field is-grouped is-grouped-right" ]
        ...
        , case findFormError model of
            Just error ->
                Html.p [ Attr.class "help content is-danger" ] [ Html.text error.message ]
...
findFieldError : Field -> Model -> Maybe Api.SignIn.Error
findFieldError field model =
    let
        hasMatchingField : Api.SignIn.Error -> Bool
        hasMatchingField error =
            error.field == Just (fromFieldToInputType field)
    in
    model.errors
